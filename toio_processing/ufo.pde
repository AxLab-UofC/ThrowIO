...
class smallSelfParticle {
  ...
  public void draw() {
    ...
    if (nextBall == true) {
      particles.add(new Particle(width-50, height, 40, -30, 100)); //this is the ball that drops from the tube
      nextBall = false;
    }
    ...
    if (hitTarget == true) {
      ...
      if (addParticle == false) {
        ...
        if(avgXVelocity > 0){
          particles.add(new Particle(hitX, 720, 40, random(10, 11), random(120, 140))); //this is where we currently define the ball speed and velocity
        }else{
          particles.add(new Particle(hitX, 720, 40, -random(10, 11), random(120, 140))); //this is where we currently define the ball speed and velocity
        }
...
void draw() {
  ...
  if (clickCount >= 4 && flag_seeBall == false && flag_ballSticks == false) {
    ...
    if (startTime == false) {
      ...
      if (millis() > time + 1500) { //wait for the ball to stick properly maybe 300 (if we always sticks, then we need to use 300 to show a smooth transition)
        startTime = false;
        ...
        if (detectBall(false)) {
          ...
          hitX = global_scaledX; //store the position of hitX (since we run many times for image detection, we may lose the hitX information)
    ...
    hitX = map(hitX, 32, 614+32, 0, width);
    ...
    if (startTime == false) {
      ...
      if (millis() > time + 2300) { //wait for virtual ball to drop
